#' Compute the marginal effects and store them in a list for the subset (girls)
#'
#' @param outcome_var 
#' @param pred_var 
...
#' @param girls 
#'
#' @return marginal_effects_list_girls
#' @export
#'
#' @examples
do_marginal_effects_girls <- function(
    outcome_var,
    pred_var,
  ...
  marginal_effects_list_girls <- list()
  ...
  for (outcome in outcome_var) {
    ...
    for (predictor in pred_var) {
      ...
      coef_summary <- coef(summary(fit_girls))
      ...
      if (adjusted_p_values[predictor] <= 0.05) {
        ...
        marginal_effects <- ggpredict(fit_girls, terms = predictor, data = girls)
        
        # Add the data frame to the list
        marginal_effects_list_girls[[paste(outcome, predictor, sep = "_")]] <- marginal_effects
      }
    }
  }
  return(marginal_effects_list_girls)
